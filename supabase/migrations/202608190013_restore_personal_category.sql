create or replace function public.update_personal_category(
  p_categoria_id uuid,
...
begin
  ...
  if not found then
    raise exception 'La categoría no pertenece al espacio personal activo del usuario.';
  end if;
...
revoke execute on function public.update_personal_category(uuid, text, boolean, boolean) from public;
revoke execute on function public.update_personal_category(uuid, text, boolean, boolean) from anon;
grant execute on function public.update_personal_category(uuid, text, boolean, boolean) to authenticated;
