...
export function ThemeToggle({ variant = 'floating' }: { variant?: 'floating' | 'inline' }) {
  ...
  return (
    <div
      className={`${variant === 'floating' ? 'fixed right-3 top-3 z-50 sm:right-5 sm:top-5' : 'w-full'} flex rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-1 shadow-sm`}
      role="group"
