  ...
  return (
    <section className="mb-10 w-full text-left md:relative md:left-1/2 md:w-[min(72rem,calc(100vw-3rem))] md:-translate-x-1/2" aria-labelledby={`monthly-summary-${spaceId}`}>
      <div className="mb-4 grid grid-cols-[1fr_auto_1fr] items-center gap-2 sm:gap-3">
        <button type="button" onClick={() => setMonth((value) => shiftMonth(value, -1))} className="justify-self-start rounded border border-gray-300 px-2 py-2 text-sm font-semibold text-gray-700 sm:px-3 sm:text-base">
          Mes anterior
        </button>
        <h2 id={`monthly-summary-${spaceId}`} className="min-w-0 text-center text-lg font-semibold text-gray-900 sm:text-2xl">
          {formatMonth(month)}
        </h2>
        <button type="button" onClick={() => setMonth((value) => shiftMonth(value, 1))} disabled={month >= currentMonth} className="justify-self-end rounded border border-gray-300 px-2 py-2 text-sm font-semibold text-gray-700 disabled:cursor-not-allowed disabled:opacity-40 sm:px-3 sm:text-base">
          Mes siguiente
        </button>
      </div>
      ...
      {!isLoading && !error && summary && summary.expenseCount > 0 && (
        ...
        <div className="flex w-full snap-x snap-mandatory gap-4 overflow-x-auto overscroll-x-contain pb-4 [scrollbar-width:none] md:grid md:grid-cols-3 md:gap-5 md:overflow-visible">
          ...
          <article className="min-h-[25rem] min-w-[88%] snap-center overflow-hidden rounded-3xl bg-violet-100 p-5 text-gray-900 shadow-sm sm:p-6 md:min-w-0">
            <h3 className="mb-4 text-center text-xl font-bold">Categorías</h3>
            <div className="mb-5 grid grid-cols-2 rounded-xl bg-white/70 p-1 text-sm" aria-label="Vista de categorías">
            ...
            {categoryView === 'distribution' ? (
              <div>
                <div className="relative mx-auto mb-5 aspect-square w-40 rounded-full sm:w-44" style={getDonutStyle(summary.categories)} role="img" aria-label="Distribución porcentual de gastos por categoría">
                  <div className="absolute inset-[24%] flex items-center justify-center rounded-full bg-violet-100 text-center text-sm font-bold">100%</div>
                </div>
              ...
              <div className="space-y-4">
                {summary.categories.map((category) => (
                  <div key={category.categoryId} className="min-w-0">
                    ...
                    <div className="h-3 overflow-hidden rounded-full bg-white" aria-label={`${category.name}: ${category.percentage}%`}><div className="h-full rounded-full bg-violet-600" style={{ width: `${Math.min(category.percentage, 100)}%` }} /></div>
                  </div>
                ))}
              </div>
            )}
          ...
          {showMembers && (
            <article className="min-h-[25rem] min-w-[88%] snap-center overflow-hidden rounded-3xl bg-emerald-100 p-5 text-gray-900 shadow-sm sm:p-6 md:min-w-0">
              <h3 className="mb-4 text-center text-xl font-bold">Integrantes</h3>
              <div className="mb-5 grid grid-cols-2 rounded-xl bg-white/70 p-1 text-sm" aria-label="Vista de integrantes">
              ...
              {memberView === 'distribution' ? (
                <div>
                  <div className="relative mx-auto mb-5 aspect-square w-40 rounded-full sm:w-44" style={getDonutStyle(summary.members)} role="img" aria-label="Distribución porcentual de pagos por integrante">
                    <div className="absolute inset-[24%] flex items-center justify-center rounded-full bg-emerald-100 text-center text-sm font-bold">100%</div>
                  </div>
                  <ul className="space-y-2">
                ...
                <div className="space-y-4">
                  {summary.members.map((member) => (
                    <div key={member.membershipId} className="min-w-0">
                      ...
                      <div className="h-3 overflow-hidden rounded-full bg-white" aria-label={`${member.name}: ${member.percentage}%`}><div className="h-full rounded-full bg-emerald-600" style={{ width: `${Math.min(member.percentage, 100)}%` }} /></div>
                    </div>
                  ))}
                </div>
              )}
          ...
          {!showMembers && summary.categories[0] && (
            <article className="flex min-h-[25rem] min-w-[88%] snap-center flex-col overflow-hidden rounded-3xl bg-amber-100 p-6 text-gray-900 shadow-sm md:min-w-0">
              <p className="mb-3 text-sm font-semibold uppercase tracking-wide text-amber-800">Destacado del mes</p>
